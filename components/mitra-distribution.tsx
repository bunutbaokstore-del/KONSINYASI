...
} from "@/lib/mitra-supply-requests";
...
export function MitraDistribution() {
  ...
  const handleSave = () => {
    try {
      saveMitraSupplyRequest(
        { productId, quantity: Number(quantity.replace(/[^0-9]/g, "")), note },
        products,
      );
      setQuantity("");
      setNote("");
      setError(null);
      setMessage(
        "Pengajuan Supply berhasil dikirim dan menunggu persetujuan Admin Distributor.",
      );
    } catch (caught) {
      setMessage(null);
      setError(
        caught instanceof Error
          ? caught.message
          : "Pengajuan tidak dapat disimpan.",
      );
    }
  };
  return (
        ...
        <View style={styles.menuList}>
          {MENUS.map((menu) => (
            ...
            >
              ...
              >
                <AppIcon name={menu.icon} size={20} color={colors.primary} />
              </View>
          ...
          {view === "products" ? (
            ...
            >
              ...
              <Text style={[styles.helper, { color: colors.muted }]}>
                Ajukan kebutuhan produk kepada Admin Distributor. Pengajuan
            ...
            <MitraProductShipment onSaved={() => selectView("history")} />
          ) : view === "receiving" ? (
            <MitraShipmentReceiving onSaved={() => selectView("history")} />
          ) : (
            <DistributionHistory
              requests={requests}
              products={products}
...
function DistributionHistory({
  ...
  requests: ReturnType<typeof useMitraSupplyRequests>;
  products: ReturnType<typeof useMitraProducts>;
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.helper, { color: colors.muted }]}>
        Pantau pengajuan supply dan status distribusi.
      </Text>
      {requests.length ? (
          ...
          >
            <View style={styles.copy}>
              <Text style={[styles.name, { color: colors.foreground }]}>
                {products.find((product) => product.id === request.productId)
  ...
  const color =
    status === "Disetujui"
      ...
      : status === "Ditolak"
        ? colors.error
        : colors.warning;
  return <Text style={[styles.status, { color }]}>{status}</Text>;
}
const styles = StyleSheet.create({
  ...
  menuText: { flex: 1, fontSize: 14, fontWeight: "800" },
  backButton: {
    minHeight: 42,
    borderWidth: 1,
    borderRadius: 12,
  ...
  card: { borderWidth: 1, borderRadius: 19, padding: 15, marginTop: 14 },
  title: { fontSize: 19, fontWeight: "800" },
  helper: { fontSize: 12, lineHeight: 18, marginTop: 5 },
  label: { fontSize: 12, fontWeight: "800", marginTop: 16, marginBottom: 7 },
  ...
  message: { fontSize: 12, fontWeight: "700", marginTop: 11 },
  historyRow: {
    flexDirection: "row",
    alignItems: "center",
  ...
  empty: { fontSize: 12, paddingVertical: 18 },
