...
function MitraRequestView({ colors, requestsQuery, onBack }: ViewProps) {
  ...
  return <RequestShell colors={colors} title="Pengajuan supplier" subtitle="Ajukan barang atau perubahan stok kepada Admin." onBack={onBack}>
    ...
    <RequestList colors={colors} requests={visibleRequests} loading={requestsQuery.isLoading} onRefresh={() => void requestsQuery.refetch()} showActions={false} />
  </RequestShell>;
}
...
function ManagerReviewView({ colors, requestsQuery, onBack }: ViewProps) {
  ...
  const executeReview = async () => {
    if (!pendingReview || reviewMutation.isPending) return;
    try {
      await reviewMutation.mutateAsync(pendingReview);
      setPendingReview(null);
      await requestsQuery.refetch();
    } catch (reviewError) {
      setError(reviewError instanceof Error ? reviewError.message : "Pengajuan belum dapat diproses.");
    }
  };
  const confirmationLabel = pendingReview?.decision === "approved" ? "Setujui" : "Tolak";
