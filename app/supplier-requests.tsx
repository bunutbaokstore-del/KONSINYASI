...
function AdminReviewView({ colors, requestsQuery, onBack, requestId, scrollRef }: ViewProps) {
  ...
  const executeAdmin = async () => {
    ...
    if (pendingAdmin.action === "reject" && !(pendingAdmin.reviewNote ?? "").trim()) {
    ...
    try {
      ...
      let freshData: SupplierRequest[] = allRequests;
      try {
        const result = await requestsQuery.refetch();
        if (result.isError) refetchFailed = true;
        else if (result.data) freshData = result.data as SupplierRequest[];
      } catch {
        refetchFailed = true;
      }
  ...
  const retryRefresh = async () => {
    if (adminReviewMutation.isPending) return;
    setError(null);
    setRetryAvailable(false);
    try {
      const result = await requestsQuery.refetch();
      if (result.isError) {
        setError("Status pengajuan belum dapat dimuat. Coba lagi.");
        setRetryAvailable(true);
        return;
      }
      await invalidateRequests();
...
function ManagerReviewView({ colors, requestsQuery, onBack }: Omit<ViewProps, "requestId" | "scrollRef">) {
  ...
  const displayRequests =
    focusedId &&
    ((requestsQuery.data ?? []) as SupplierRequest[]).some(
      (request) => request.id === focusedId && request.status === "pending",
    )
      ? ((requestsQuery.data ?? []) as SupplierRequest[]).filter((request) => request.id === focusedId)
      : activeRequests;
