...
type ProductionView = "list" | "budget" | "results" | "hpp" | "history" | "stock" | "recap" | "shipment" | "receiving";
...
export default function ProductionScreen() {
  ...
  const [isResultSelectorOpen, setResultSelectorOpen] = useState(true);
        ...
        ListHeaderComponent={
          <View>
            ...
            <View style={[styles.segment, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <Pressable accessibilityRole="button" onPress={() => switchView("budget")} style={[styles.segmentButton, view === "budget" && { backgroundColor: colors.primary }]}><Text style={[styles.segmentText, { color: view === "budget" ? colors.background : colors.muted }]}>Anggaran</Text></Pressable>
              <Pressable accessibilityRole="button" onPress={() => switchView("list")} style={[styles.segmentButton, view === "list" && { backgroundColor: colors.primary }]}><Text style={[styles.segmentText, { color: view === "list" ? colors.background : colors.muted }]}>Daftar</Text></Pressable>
              <Pressable accessibilityRole="button" onPress={() => switchView("hpp")} style={[styles.segmentButton, view === "hpp" && { backgroundColor: colors.primary }]}><Text style={[styles.segmentText, { color: view === "hpp" ? colors.background : colors.muted }]}>HPP</Text></Pressable>
              <Pressable accessibilityRole="button" onPress={() => switchView("results")} style={[styles.segmentButton, view === "results" && { backgroundColor: colors.primary }]}><Text style={[styles.segmentText, { color: view === "results" ? colors.background : colors.muted }]}>Hasil</Text></Pressable>
...
function ResultForm({ colors, productions, productNames, selectedProduction, isSelectorOpen, onToggleSelector, onSelectProduction, actualQuantity, onActualChange, damagedQuantity, onDamagedChange, resultNotes, onNotesChange, onSave, error, savedMessage }: { colors: Colors; productions: MitraProduction[]; productNames: Map<string, string>; selectedProduction: MitraProduction | null; isSelectorOpen: boolean; onToggleSelector: () => void; onSelectProduction: (id: string) => void; actualQuantity: string; onActualChange: (value: string) => void; damagedQuantity: string; onDamagedChange: (value: string) => void; resultNotes: string; onNotesChange: (value: string) => void; onSave: () => void; error: string | null; savedMessage: string | null }) {
  ...
  return <View style={[styles.formCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
    ...
    <View style={styles.selectorHeader}><Text style={[styles.fieldLabel, { color: colors.foreground }]}>Pilih Produksi</Text><Pressable accessibilityRole="button" onPress={onToggleSelector} style={[styles.selectorToggle, { borderColor: colors.border }]}><Text style={[styles.selectorToggleText, { color: colors.primary }]}>{isSelectorOpen ? "Tutup" : "Buka Daftar"}</Text></Pressable></View>
    <Text style={[styles.helper, { color: colors.muted }]}>{productLabel}</Text>
    {isSelectorOpen ? <View style={styles.optionList}>{productions.map((production) => <ProductionSelectOption key={production.id} production={production} productName={productNames.get(production.productId) ?? "Produk tidak ditemukan"} selected={production.id === selectedProduction?.id} onSelect={() => onSelectProduction(production.id)} colors={colors} />)}</View> : null}
    ...
    <View style={[styles.readOnlyInput, { backgroundColor: colors.background, borderColor: colors.border }]}><Text style={[styles.readOnlyText, { color: selectedProduction ? colors.foreground : colors.muted }]}>{selectedProduction ? `${selectedProduction.targetQuantity} unit` : "Jumlah dari Daftar Produksi"}</Text></View>
...
function FilterChip({ label, selected, onPress, colors }: { label: string; selected: boolean; onPress: () => void; colors: Colors }) {
  return <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => [styles.filterChip, { borderColor: selected ? colors.primary : colors.border, backgroundColor: selected ? `${colors.primary}14` : colors.background }, pressed && styles.pressed]}><Text style={[styles.filterText, { color: selected ? colors.primary : colors.muted }]}>{label}</Text></Pressable>;
}

function HistoryCard({ production, productName, hpp, colors }: { production: MitraProduction; productName: string; hpp?: MitraProductionHpp; colors: Colors }) {
  return <View style={[styles.historyCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={styles.productionTop}><View style={styles.productionTitleCopy}><Text style={[styles.productionName, { color: colors.foreground }]}>{productName}</Text><Text style={[styles.productionDate, { color: colors.muted }]}>{production.productionDate}</Text></View><View style={[styles.statusBadge, { backgroundColor: production.status === "Selesai" ? `${colors.success}18` : `${colors.warning}18` }]}><Text style={[styles.statusText, { color: production.status === "Selesai" ? colors.success : colors.warning }]}>{production.status}</Text></View></View><View style={styles.detailGrid}><Detail label="Jumlah Produksi" value={`${production.actualQuantity ?? "-"} unit`} colors={colors} /><Detail label="Hasil Aktual" value={production.actualQuantity === null ? "Belum tersedia" : `${production.actualQuantity} unit`} colors={colors} /><Detail label="HPP Produksi" value={hpp ? formatCurrency(hpp.costPerUnit) : "Belum diatur"} colors={colors} /><Detail label="Persentase Hasil" value={production.yieldPercentage === null ? "-" : `${production.yieldPercentage}%`} colors={colors} /></View>{production.damagedQuantity !== null ? <Text style={[styles.historyMeta, { color: colors.muted }]}>Rusak / Susut: {production.damagedQuantity} unit</Text> : null}{production.resultNotes ? <Text style={[styles.notes, { color: colors.muted }]}>Catatan hasil: {production.resultNotes}</Text> : null}</View>;
}
...
function BudgetForm({ colors, products, selectedProductId, onSelectProduct, period, onSelectPeriod, periodDate, onPeriodDateChange, budgetValue, onBudgetChange, target, onTargetChange, onSave, error, savedMessage, savedBudget }: { colors: Colors; products: ReturnType<typeof useMitraProducts>; selectedProductId: string | null; onSelectProduct: (id: string) => void; period: BudgetPeriod; onSelectPeriod: (period: BudgetPeriod) => void; periodDate: string; onPeriodDateChange: (date: string) => void; budgetValue: string; onBudgetChange: (value: string) => void; target: string; onTargetChange: (value: string) => void; onSave: () => void; error: string | null; savedMessage: string | null; savedBudget?: { period: BudgetPeriod; productionBudget: number; productionTarget: number } }) {
  ...
  const periodLabel = periodChosen ? formatBudgetPeriodSelection(period, periodDate) : "Periode Anggaran";
  return <View style={[styles.formCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
    ...
    <View style={styles.optionList}>{products.map((product) => <BudgetProductOption key={product.id} product={product} selected={product.id === selectedProductId} onSelect={() => onSelectProduct(product.id)} colors={colors} compact />)}</View>
    <Pressable accessibilityRole="button" accessibilityLabel="Pilih periode anggaran" onPress={() => setPeriodMenuOpen((open) => !open)} style={({ pressed }) => [styles.periodField, { borderColor: periodMenuOpen || periodChosen ? colors.primary : colors.border, backgroundColor: colors.background }, pressed && styles.pressed]}><Text style={[styles.periodFieldText, { color: periodChosen ? colors.foreground : colors.muted }]}>{periodLabel}</Text><Text style={[styles.periodChevron, { color: periodMenuOpen || periodChosen ? colors.primary : colors.muted }]}>▼</Text></Pressable>
    {periodMenuOpen ? <View style={[styles.periodMenu, { backgroundColor: colors.background, borderColor: colors.border }]}>{(["Hari", "Minggu", "Bulan"] as BudgetPeriod[]).map((option) => <Pressable key={option} accessibilityRole="button" onPress={() => { onSelectPeriod(option); setPeriodChosen(true); setPeriodMenuOpen(false); }} style={[styles.periodOption, { backgroundColor: period === option && periodChosen ? `${colors.primary}14` : "transparent" }]}><Text style={[styles.periodOptionText, { color: period === option && periodChosen ? colors.primary : colors.foreground }]}>{option}</Text>{period === option && periodChosen ? <AppIcon name="verified" size={17} color={colors.primary} /> : null}</Pressable>)}</View> : null}
    {periodChosen ? <PeriodCalendar period={period} dateKey={periodDate} onDateChange={onPeriodDateChange} colors={colors} /> : null}
...
function PeriodCalendar({ period, dateKey, onDateChange, colors }: { period: BudgetPeriod; dateKey: string; onDateChange: (date: string) => void; colors: Colors }) {
  const [monthKey, setMonthKey] = useState(dateKeyToMonthKey(dateKey));
  const days = getCalendarMonthDays(monthKey);
  const monthLabel = formatBudgetPeriodSelection("Bulan", `${monthKey}-01`);
  const weekdays = ["Sen", "Sel", "Rab", "Kam", "Jum", "Sab", "Min"];
  return <View style={[styles.calendar, { backgroundColor: colors.background, borderColor: colors.border }]}>
    <View style={styles.calendarHeader}>
      <Pressable accessibilityRole="button" accessibilityLabel="Bulan sebelumnya" onPress={() => setMonthKey(shiftCalendarMonth(monthKey, -1))} style={[styles.calendarNav, { borderColor: colors.border }]}><AppIcon name="chevron-left" size={18} color={colors.primary} /></Pressable>
      <Text style={[styles.calendarTitle, { color: colors.foreground }]}>{monthLabel}</Text>
      <Pressable accessibilityRole="button" accessibilityLabel="Bulan berikutnya" onPress={() => setMonthKey(shiftCalendarMonth(monthKey, 1))} style={[styles.calendarNav, { borderColor: colors.border }]}><AppIcon name="chevron-right" size={18} color={colors.primary} /></Pressable>
    </View>
    <Text style={[styles.periodSummary, { color: colors.primary }]}>{period === "Bulan" ? `Bulan terpilih: ${formatBudgetPeriodSelection(period, dateKey)}` : period === "Minggu" ? `Minggu terpilih: ${formatBudgetPeriodSelection(period, dateKey)}` : `Tanggal terpilih: ${formatBudgetPeriodSelection(period, dateKey)}`}</Text>
    <View style={styles.weekdayRow}>{weekdays.map((weekday) => <Text key={weekday} style={[styles.weekday, { color: colors.muted }]}>{weekday}</Text>)}</View>
    <View style={styles.calendarGrid}>{days.map((day) => { const inMonth = dateKeyToMonthKey(day) === monthKey; const selected = isDateInSelectedPeriod(period, dateKey, day); return <Pressable key={day} accessibilityRole="button" accessibilityLabel={`Pilih ${day}`} onPress={() => { setMonthKey(dateKeyToMonthKey(day)); onDateChange(day); }} style={[styles.calendarDay, { backgroundColor: selected ? `${colors.primary}18` : "transparent", borderColor: selected ? colors.primary : "transparent" }]}><Text style={[styles.calendarDayText, { color: inMonth ? colors.foreground : colors.muted, opacity: inMonth ? 1 : 0.45 }]}>{parseDateKey(day).getDate()}</Text></Pressable>; })}</View>
  </View>;
}
