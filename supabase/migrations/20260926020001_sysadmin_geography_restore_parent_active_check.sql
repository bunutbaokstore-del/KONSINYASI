-- =============================================================================
-- SYSADMIN MASTER GEOGRAPHY - RESTORE PARENT ACTIVE CHECK
-- This migration restores parent is_active validation on create RPCs
-- that was lost due to migration ordering (20260922133118 ran before 20260926020000)
-- Run AFTER 20260926020000_sysadmin_master_geography_rpcs.sql
-- =============================================================================

-- =============================================================================
-- 1. RESTORE CREATE KABUPATEN/KOTA - Check Provinsi is_active
-- =============================================================================
...
begin
  ...
  if not v_provinsi.is_active then
    raise exception using errcode = '22003', message = 'Tidak dapat membuat Kabupaten/Kota di Provinsi yang tidak aktif.';
  end if;
...
-- =============================================================================
-- 2. RESTORE CREATE KECAMATAN - Check Kabupaten/Kota is_active
-- =============================================================================
...
begin
  ...
  if not v_kab.is_active then
    raise exception using errcode = '22003', message = 'Tidak dapat membuat Kecamatan di Kabupaten/Kota yang tidak aktif.';
  end if;
...
-- =============================================================================
-- 3. RESTORE CREATE DESA - Check Kabupaten/Kota and Kecamatan is_active
-- =============================================================================
...
begin
  ...
  -- Validate parent Kabupaten/Kota exists and is active
  select * into v_kab from public.kabupaten_kota where id = p_kabupaten_kota_id;
  if not found then
    raise exception using errcode = 'P0002', message = 'Kabupaten/Kota tidak ditemukan.';
  end if;
  ...
  if not v_kab.is_active then
    raise exception using errcode = '22003', message = 'Tidak dapat membuat Desa di Kabupaten/Kota yang tidak aktif.';
  end if;

  -- Validate parent Kecamatan exists and is active
  ...
  if not v_kec.is_active then
    raise exception using errcode = '22003', message = 'Tidak dapat membuat Desa di Kecamatan yang tidak aktif.';
  end if;
