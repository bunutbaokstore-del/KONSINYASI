...
async function cleanupFixture() {
  ...
  const requestIds = [requestAId, requestBId, rejectedRequestId].filter(Boolean);
  ...
  const { data: items } = await admin.from("consignment_items").select("id").in("mitra_user_id", [fixture[3].id, fixture[4].id]);
  ...
  const { error: productsError } = await admin.from("products").delete().in("created_by_mitra_user_id", [fixture[3].id, fixture[4].id]);
